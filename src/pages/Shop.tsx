...
const CATS: string[] = ["All", "Men", "Women"];
...
const pct = (p: number, o: number): number => Math.round((1 - p / o) * 100);
    ...
    <div className="shop-root">
      <style>{`
        ...
        .p-card {
          ...
          border-radius: 10px; overflow: hidden; transition: .3s;
        }
        .p-card:hover { transform: translateY(-5px); box-shadow: 0 10px 30px rgba(0,0,0,.08); }
        .product-grid.list-mode .p-card {
          display: grid; grid-template-columns: 200px 1fr;
        }
        .p-card-img-wrap { position: relative; height: 260px; overflow: hidden; }
        .product-grid.list-mode .p-card-img-wrap { height: 200px; }
        ...
        .p-actions {
          ...
          display: flex; transform: translateY(100%); transition: .3s;
        }
        ...
        .btn-wish  { width: 46px; background: #fff; }
        .btn-wish.wished { background: #e63946; color: #fff; }
        .p-card-body { padding: 16px; }
        .p-cat-label { font-size: 10px; color: #e63946; margin-bottom: 4px; }
        .p-name  { font-size: 15px; font-weight: 700; margin-bottom: 6px; }
        .p-price-row { display: flex; gap: 8px; align-items: center; margin-top: 6px; }
        .p-price { font-weight: 800; font-size: 18px; }
        .p-orig  { font-size: 12px; text-decoration: line-through; color: #aaa; }
        .p-colors { display: flex; gap: 6px; margin-top: 8px; }
        .p-color  { width: 12px; height: 12px; border-radius: 50%; border: 1px solid #ddd; }
        .cart-toast {
          position: fixed; bottom: 20px; right: 20px;
          background: #111; color: #fff;
          padding: 12px 20px; border-radius: 6px;
          opacity: 0; transform: translateY(40px); transition: .3s;
        }
            ...
            {filtered.map((prod: Product) => (
              <div key={prod.id} className="p-card">
                ...
                <div className="p-card-img-wrap">
                  <img src={prod.img} alt={prod.name} className="p-card-img" />
